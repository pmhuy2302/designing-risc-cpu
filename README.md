...
| `ld_ac` | 0 | 0 | 0 | 0 | 0 | 0 | 0 | ALUOP | Latches **ALU** computation result into **AC** during `STORE` phase. |
...
* **System Clock Generator (*always #5 clk = ~clk;*)**: Establishes a clock of 100 MHz clock (10 ns time period).

* **The ***test_case*** Index**: An internal tracking variable used to identify suitable display
mode to the TCL console.

* **Memory Clear Task (*clear_ram*)**: A task used for zeroing out all 32 memory addresses (***uut.mem.ram[i] = 8’b0***) before loading a new test.

* **Data Setup Task (*setup*)**: A task used for initializing program instructions and
data from an input ***text file*** customized for each testcase.

* **TCL Console Monitoring**: Conditional blocks to ensure convenient visibility. When
***test_case == 1***, the console logs signal changes to achieve this test’s aim. For
other tests (***test_case != 1***), the TCL console prints out time, current test case,
controller state, opcode, accumulator value, and the halt flag.
...
| Test ID | Test Case Name | Verification Scope | Tested Instructions | Test Objective & Pass Criteria | Status |
| :---: | :--- | :--- | :--- | :--- | :---: |
...
| **TC_02** | **ALU Logic Test** | ALU Data Path & AC Register | `LDA`, `ADD`, `AND`, `XOR`, `SUB`, `MUL`, `FPU_MUL` | Accurate integer & 9-bit calculation and writeback | **PASSED** |
| **TC_03** | **Special Logic Operations** | Branching, Flags & Memory Write | `JMP`, `SKZ`, `SKO`, `STO`, `SHL`, `SHR`, `OR` | Correct target jump, flag-based skipping, and RAM store | **PASSED** |
| **TC_04** | **Basic Combinational Test** | Data Pipeline & Bus Stability | `LDA`, `ADD`, `STO`, `HALT` | Stable bus transfer during Read-Compute-Write cycle | **PASSED** |
| **TC_05** | **Complex Combinational Test** | Dynamic Workload & Edge Cases | Full ISA (16 Instructions) | Zero logic/timing hazards under dynamic workload | **PASSED** |
